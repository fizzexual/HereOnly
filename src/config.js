...
/**
 * Configuration helpers: secret persistence, config-file / env loading, and
 * small CLI value coercions. Zero deps.
 */

const fs = require('node:fs');
const path = require('node:path');
const { generateSecret } = require('./core/tokens.js');

/**
 * Load a 256-bit secret from `file`, creating and persisting one (hex, mode
 * 0600) if it does not exist — keeping issued session tokens valid across
 * restarts. The parent directory is created if needed. With no file, returns
 * an ephemeral secret.
 */
function loadOrCreateSecret(file) {
  ...
  try {
    ...
    /* non-fatal: keep the ephemeral secret in memory */
  }
  return secret;
}
...
function fromEnv(env = process.env) {
  ...
  if (env.HEREONLY_AUDIT_FILE) out.auditFile = env.HEREONLY_AUDIT_FILE;
